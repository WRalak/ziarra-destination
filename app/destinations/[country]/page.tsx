    ...
    <div className="min-h-screen bg-ink text-cream">
      ...
      <main>
        <nav aria-label="Breadcrumb" className="mx-auto max-w-7xl px-4 py-4 text-sm text-sand sm:px-6 lg:px-8">
          <ol className="flex flex-wrap items-center gap-2"><li><Link href="/" className="rounded hover:text-cream focus:outline-none focus-visible:ring-2 focus-visible:ring-gold">Home</Link></li><li aria-hidden="true">/</li><li><Link href="/destinations" className="rounded hover:text-cream focus:outline-none focus-visible:ring-2 focus-visible:ring-gold">Destinations</Link></li><li aria-hidden="true">/</li><li aria-current="page" className="text-cream">{destination.name}</li></ol>
        </nav>
        <CountryHero destination={destination} />
        <section className="mx-auto grid max-w-7xl gap-5 px-4 py-10 sm:grid-cols-3 sm:px-6 lg:px-8"><div className="rounded-xl border border-line bg-surface p-5"><span className="text-xs text-sandDim">Capital</span><p className="mt-1 font-semibold">{destination.capital}</p></div><div className="rounded-xl border border-line bg-surface p-5"><span className="text-xs text-sandDim">Region</span><p className="mt-1 font-semibold">{destination.region}</p></div><div className="rounded-xl border border-line bg-surface p-5"><span className="text-xs text-sandDim">Catalogue</span><p className="mt-1 font-semibold">10 demo listings</p></div></section>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8"><p className="rounded-xl border border-gold/20 bg-gold/10 p-4 text-sm text-gold">Preview data — live availability and pricing will be connected later.</p></div>
